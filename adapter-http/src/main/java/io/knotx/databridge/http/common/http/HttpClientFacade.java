...
public class HttpClientFacade {
  ...
  private final List<HttpDataSourceSettings> services;

  private final WebClient webClient;

  private final CustomHttpHeader customHttpHeader;

  public HttpClientFacade(WebClient webClient, HttpDataSourceAdapterOptions configuration) {
    this.webClient = webClient;
    this.services = configuration.getServices();
    this.customHttpHeader = configuration.getCustomHttpHeader();
  }

  public Single<ClientResponse> process(DataSourceAdapterRequest message, HttpMethod method) {
  ...
  /**
   * Method to validate contract or params JsonObject for the DataSourceAdapterProxy Service<br>
   * The contract checks if all required fields exists in the object.
   * throwing IllegalArgumentException in case of contract violation.<br>
   *
   * @param message - Event Bus Json Object message that contains 'clientRequest' and 'params'
   *                objects.
   */
  protected void validateContract(DataSourceAdapterRequest message) {
  ...
  private Pair<ClientRequest, HttpDataSourceSettings> prepareRequestData(DataSourceAdapterRequest adapterRequest) {
    final Pair<ClientRequest, HttpDataSourceSettings> serviceData;
    ...
    final Optional<HttpDataSourceSettings> serviceMetadata = findServiceMetadata(serviceRequest.getPath());

    if (serviceMetadata.isPresent()) {
      final HttpDataSourceSettings metadata = serviceMetadata.get();
      ...
      throw new UnsupportedDataSourceException(error);
    }
    return serviceData;
  }

  private Optional<HttpDataSourceSettings> findServiceMetadata(String servicePath) {
  ...
  private void updateRequestQueryParams(HttpRequest<Buffer> request, HttpDataSourceSettings metadata) {
