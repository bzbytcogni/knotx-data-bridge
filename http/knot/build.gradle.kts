...
// -----------------------------------------------------------------------------
dependencies {
  api(platform("io.knotx:knotx-fragments-handler-api:${project.version}"))
}
...
publishing {
  publications {
    create<MavenPublication>("mavenJava") {
      artifactId = "knotx-knot-http"
      from(components["java"])
      artifact(tasks["sourcesJar"])
      artifact(tasks["javadocJar"])
      pom {
        name.set("Knot.x Data Bridge: HTTP Knot")
        description.set("Knot that allows to fetch JSON content from Web API.")
        ...
        scm {
          connection.set("scm:git:git://github.com/Knotx/knotx-data-bridge.git")
          developerConnection.set("scm:git:ssh://github.com:Knotx/knotx-data-bridge.git")
          url.set("http://knotx.io")
        }
      }
    }
